...
def format_env_dict(environ, quoted=True):
    """Format an environment dict for printing to console.

    :param quoted: If True, will use shlex.quote to quote the elements, and
    separate them with spaces. Otherwise, they will not be quoted, and will be
    separated with line breaks.
    :type quoted: bool
    """

    env_strs = ['{}={}'.format(k, v) for k, v in environ.items()]

    if quoted:
        return ' '.join([cmd_quote(s) for s in env_strs])
    else:
        return '\n'.join(env_strs)


def parse_env_str(environ_str):
    """Parse a quoted environment string generated by format_env_dict(..., quoted=True)"""

    try:
        return {k: v for k, v in [e.split('=', 1) for e in cmd_split(environ_str)]}
    except ValueError:
